    ...
    //     <li className={`px-0 l:px-2 py-[11px] hover:bg-[#ffffff1a] border border-none rounded-lg cursor-pointer ${isMore ? 'mt-[60px] relative' : ''} ${isLogo ? 'mb-[20px] py-[20px]' : ''}`}>
    ...
    //     const [showLogout, setShowLogout] = useState(false); // Manage logout button visibility
    ...
    //                     {showLogout && (<button className="absolute bottom-0 bg-red-600 text-white mt-2" onClick={handleLogout}>Log out</button>)}
    //                 </ul>
    //             </nav>
    //         </div>
...
const MenuItem: React.FC<MenuItemProps> = ({ href, Icon, label, isProfile, isMore, isLogo, onClick }) => (
  <li
    className={`relative px-0 l:px-2 py-[11px] hover:bg-[#ffffff1a] border border-none rounded-lg cursor-pointer ${
      isMore ? 'mt-[50px]' : ''
    } ${isLogo ? 'mb-[20px] py-[20px]' : ''}`}
    onClick={onClick}
  >
    <Link href={href} className="gap-0 justify-center l:gap-[15px] l:justify-normal flex items-center">
      {isProfile ? (
...
export default function SideMenu() {
  ...
  return (
    <>
      <div className="hidden l:block w-[23.5%] border-[#262626] border-r-[1px] relative"> {/* Add relative here */}
        <div className="Logo">
          <Link href="#">
            <Image src={InstaLogo} alt="Instagram logo" className="w-[136px] h-[89px] mt-2 ml-2" />
          </Link>
