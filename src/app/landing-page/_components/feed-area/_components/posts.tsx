...
const PostCommentSection: React.FC<PostCommentSectionProps> = ({ postId }) => {
    ...
    const toggleComments = () => {
        ...
        if (!showAllComments) fetchCommentsFromAPI();
    };
    ...
    return (
        <div className="mt-2">
            <p onClick={toggleComments} className="text-sm font-light text-[#a8a8a8] cursor-pointer">
                {showAllComments ? 'Hide comments' : 'View all comments'}
            </p>
            {showAllComments && comments.map((comment) => (

                <div key={comment.username + comment.content} className="text-white text-sm mb-2">
            ...
            <div className="flex justify-between items-center mt-2">
                <input
                    ...
                    className="bg-black outline-none text-white text-sm border-none w-2/3"
                />
                <button onClick={handleCommentSubmit} className="text-sm text-[#a8a8a8] hover:opacity-75">Post</button>
            </div>
        </div>
    );
};
...
const PostContent: React.FC<PostContentProps> = ({postId, profile_picture, username, time, postImage, likes, caption, setData, isClickedModal, index, ownPost, deleteFun,}) => {
